function [writedir] = OverlapFinderFull(paramfile)
%% Overlap finder
...
plot(promsmat1(:,r),'.-'), xlabel('proj #'), ylabel('peak prominence')
subplot(212)
plot(locsmat1(:,r),'.-'), xlabel('proj #'), ylabel('peak position')
...
%% 2.0 Loop over height steps, compare inner ring images to find height stitch positions
fprintf(['Finding height step stitching positions...\n']);
thisRingnames = ringnames(:,1);

%% 2.1 Flats/darks loading
% using mean flat and median dark
mflats = zeros(datsize(1),datsize(2),nhs);
mddarks = zeros(datsize(1),datsize(2),nhs);
for i = 1:nhs
    tmp = rot90(h5read([rawbasedir thisRingnames{i} filesep 'pre_ref.nxs'],'/ref_0'));
    tmp1 = rot90(h5read([rawbasedir thisRingnames{i} filesep 'post_ref.nxs'],'/ref_0'));
    flats = double(cat(3,tmp,tmp1)); clear tmp tmp1
...
% get stitch position estimates from the motors
tzpos = tzpos_all(:,1);
pixshifts = round(abs(tzpos(h)-tzpos(h+1))/pixsize_mm);
...
for p = 1:length(prange)
    tmp = rot90(double(h5read([rawbasedir thisRingnames{h} filesep thisRingnames{h} '.nxs'],h5ImagePath,...
        [1,1,prange(p)],[datsize(1),datsize(2),1])))-mddarks(:,:,h);
    imr1 = filtfunc(tmp./(mflats(:,:,h)-mddarks(:,:,h)));
    tmp = rot90(double(h5read([rawbasedir thisRingnames{h+1} filesep thisRingnames{h+1} '.nxs'],h5ImagePath,...
        [1,1,prange(p)],[datsize(1),datsize(2),1])))-mddarks(:,:,h+1);
    imr2 = filtfunc(tmp./(mflats(:,:,h+1)-mddarks(:,:,h+1)));
